...
| 3 | Renter premium tier + AI Alert Plus | Backend done | 1 | `User.subscription_status/tier/started_at/expires_at/moyasar_card_token` (migration `b3c4d5e6f7a8`). `app.core.moyasar` generalized (mediator_id-specific params → generic `metadata`/`success_url`/`back_url`) so both mediators.py and the new `subscriptions.py` reuse the same gateway; `payments.py` webhook handlers branch on `metadata.user_id` vs `metadata.mediator_id`. Routes at `/api/subscriptions`: `GET /me`, `POST /me/subscribe`, `POST /me/renew`, `POST /me/unsubscribe` (immediate cancel, not end-of-period), `GET /premium-status` (demo gated route). `get_premium_user` gate in `deps.py` mirrors `get_mediator_user`, with lazy expiry flip (`active` → `expired` on read past `subscription_expires_at`, mirrors `contracts.py`'s pattern). 9 new tests in `tests/test_subscriptions.py`, full suite (185) green. **Not done:** no AI Alert Plus / unlimited AI Advisor chat wiring yet — `get_premium_user` exists but isn't yet applied to `ai.py` or saved-search alert delivery; frontend/mobile UI. |
